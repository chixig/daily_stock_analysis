...
## 2026-09-16 B09 start
Predeclared8 intraday-path hypotheses and late/overnight decomposition before results. No primary direction switch or fresh-OOS claim. See frozen spec.
...
## 2026-09-16 B10 结果导向研究启动
按用户新要求建立净>=1%案例库、对照与浅层分类/回归树。参数在首次运行前冻结，全部计算GitHub，样本内发现与逐年回溯明确区分，不引入新交易授权。

## 2026-09-16 B10结果与成交复核完成
...
## 2026-09-16 B11启动
用户指出删赢家与删亏损应对称检验，并明确批准两路线。保持B10阈值，风险树每年训练，止损正确处理跳空与成交代理；首次运行前冻结。

## 2026-09-16 B11两路线完成与分币成交修正
...
## 2026-09-17 B12启动
用户按计划继续；先执行与事件验证，不调C/止损/期限。分钟快照与OHLC严格区分，缺覆盖明确披露。

## 2026-09-17 B12完成现有数据检验
